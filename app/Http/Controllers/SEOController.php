...
public function generateWallpaperPrompt(int $id): JsonResponse
{
    ...
    $charaWithFranchise = $wallpaper->characters->map(function ($character) use (&$characterNames, &$franchiseNames) {
        ...
        // Default ke NTE jika series tidak ada di DB, mengingat ini web spesifik NTE
        $franchiseName = $character->series->first()?->name ?? 'Neverness to Everness';
    ...
    // Penyesuaian fallback default untuk NTE
    $rawCharacters = $characterNames ? implode(', ', $characterNames) : (in_array('Scenery', $tagsArray) ? 'NTE Landscape' : 'NTE Character');
    $rawFranchises = $franchiseNames ? implode(', ', $franchiseNames) : 'Neverness to Everness';
    ...
    if ($wallpaper->width > $wallpaper->height) {
        ...
        $deviceTarget = 'PC / PS5 / Desktop'; // Pemain NTE sering mencari untuk platform ini
    ...
    $prompt = <<<EOT
You are a real, passionate human player running a popular Neverness to Everness (NTE) wallpaper platform. Your writing MUST pass as 100% human-written and bypass AI detectors (score 85%+ human). 
...
- LORE INJECTION: You MUST casually drop 1 specific NTE jargon/term in the image_description (e.g., Anomalies, the city of Hethereau, Esper abilities, vehicle customization, mint properties, or urban supernatural phenomena) to prove you actually play the game. Ensure the term visually matches the character/scene.
...
1. _image_analysis:
   DO THIS FIRST. Perform a hyper-detailed visual extraction focusing on Subject Micro-Details, Apparel & Textures, and Environment & Atmosphere. Identify any NTE-specific visual cues (urban environments, specific character weapons, customized cars, or anomaly signs).
...
3. image_alt (Max 125 characters):
   ...
   - Formula: [Subject micro-action] + [Clothing texture/color] + [Character] from Neverness to Everness + [Specific Background/Lighting].
...
5. seo_description (145-155 characters):
   BASED STRICTLY ON YOUR _image_analysis. Write it like a casual, direct recommendation for fellow NTE players, focusing on the most striking visual element.
...
6. image_description (80-120 words):
   ...
   - Casually drop the NTE lore term here and credit {$artists} if known.
   - End with a quick, practical reason why this specific layout is a god-tier {$deviceTarget} wallpaper (e.g., "dark background saves battery during night drives", "leaves perfect room for your apps").
...
8. seo_keywords (comma-separated string):
   ...
   - Core Intent: "{$rawCharacters} neverness to everness wallpaper {$resolution}", "nte {$rawCharacters} wallpaper"
   - Device Intent: "{$rawCharacters} {$deviceTarget} background", "neverness to everness {$orientation} wallpaper"
   - Aesthetic Intent: [Dominant Color/Vibe] + "{$rawCharacters} art"
   - Action Intent: "download {$rawCharacters} wallpaper", "clean {$rawCharacters} nte background"
...
public function generateCharacterPrompt(int $id): JsonResponse
{
    ...
    // Fallback otomatis ke Neverness to Everness
    $franchiseName = $character->series->first()?->name ?? 'Neverness to Everness';
    
    // Sesuaikan default keyword dengan niche NTE
    $characterKeywords = $character->keywords ?? 'nte aesthetic, 4k neverness to everness';
    
    // Opsional: Jika DB kamu punya field role/kemampuan, bisa di-mapping di sini. 
    // Sementara pakai default yang memancing LLM mencari Role/Esper type karakter.
    $characterRole = $character->role ?? 'Neverness to Everness Character (Esper/Role)'; 
    $visualTraits = $character->visual_traits ?? 'iconic urban design, vehicle and weapon aesthetics';

    $prompt = <<<EOT
You are an authentic, passionate player running a personal Neverness to Everness (NTE) wallpaper blog. Your writing MUST pass as 100% human-written and bypass AI detectors (score 85%+ human).

CRITICAL MISSION: You are writing high-CTR SEO metadata and page content for an IMAGE GALLERY PAGE displaying wallpapers of {$characterName} from Neverness to Everness.
...
1. _character_analytic:
   Briefly analyze why NTE fans like {$characterName}'s design based on their {$visualTraits} or in-game animations/ultimate abilities.
...
2. seo_title (50-60 characters):
   ...
   - ABBREVIATIONS: Always use "NTE" instead of "Neverness to Everness" to save space and match search intent.
   ...
   - EXAMPLES OF GOOD FRONT-LOADED TITLES:
     "{$characterName} NTE 4K Wallpapers"
     "{$characterName} NTE Wallpapers 4K"
     "{$characterName} Neverness To Everness HD & 4K Fanart"
     "{$characterName} NTE Aesthetic Wallpapers HD"
...
4. seo_keywords:
   Generate 12 intent-driven long-tail keywords. MUST include combinations of "{$characterName}", "nte", "neverness to everness", "wallpaper", "4k", "mobile", "pc". ALL LOWERCASE. Comma separated.

5. atomic_answer (40-50 words):
   Highly factual Entity-Relationship statement for AI Overviews. 
   - Format: "{$characterName} is a [Role/Esper Type] character in Neverness to Everness, recognized by [Visual Traits]. This page provides a gallery of high-resolution desktop and mobile wallpapers featuring the character."
...
6. description (150-200 words): 
   ...
   - Paragraph 1 (The Lore Hook): Explain WHO {$characterName} is in {$franchiseName}. You MUST mention their in-game Role (e.g., Esper, Anomaly Hunter, Business Owner) or Faction. Mentally roll a dice and pick ONE of these angles:
     * Angle 1 (The Meta Focus): Talk about their gameplay utility, vehicle synergy, or banner hype, then touch on their design.
     * Angle 2 (The Story Impact): Focus heavily on their personality/lore vibe in the city of Hethereau or anomalous zones.
     * Angle 3 (The Pure Aesthetic Hook): Start immediately by geeking out over their animations, urban outfits, or vehicle/weapon design.
...
ENTITY CONTEXT:
- Tag Name: {$tagName}
- Note: This tag could represent a Character, an NTE Role/Ability (e.g., Esper, Anomaly Hunter), an aesthetic (e.g., cyberpunk, urban fantasy, car culture), a Faction (e.g., Store Owners, Syndicates), a Location (e.g., Hethereau, Anomalous zones), or a visual trait. 
- You must deduce its exact context within the Neverness to Everness universe and adapt your tone accordingly.
...
1. _tag_analysis:
   Briefly deduce what "{$tagName}" is in the context of Neverness to Everness (Is it a Role? Location? Vehicle? Aesthetic? Color theme?). Why do NTE fans search for this specific visual?
...
2. seo_title (40-60 characters):
   ...
   CRITICAL RULES:
   - DYNAMIC STRUCTURE: Rearrange the modifiers (NTE, Neverness to Everness, HD, 4K, PC, Mobile) naturally.
   ...
   - CONTEXTUAL INTEGRATION: You must include "NTE" or "Neverness to Everness" naturally if space permits.
   - EXAMPLES OF HUMAN-LIKE TITLES:
     "Clean {$tagName} NTE Wallpapers 4K"
     "My {$tagName} Neverness To Everness Stash for PC & Mobile"
     "{$tagName} NTE Backgrounds and Fanart Dump"
     "Chill {$tagName} Wallpapers in High Res"
     "{$tagName} NTE Art (4K Backgrounds)"
...
4. seo_keywords:
   Generate 12 intent-driven long-tail keywords. MUST include combinations with "nte", "neverness to everness", "mobile", and "pc". ALL LOWERCASE. Comma separated.

5. atomic_answer (40-50 words):
   Highly factual Entity statement designed for Google AI Overviews.
   - Format: "{$tagName} in the context of Neverness to Everness refers to [Brief Definition]. This page provides a curated gallery of high-resolution desktop and mobile wallpapers matching this specific theme or in-game element."
...
6. description (150-200 words): 
   ...
   - Paragraph 1 (The Vibe Hook): Mentally roll a dice and pick ONE of these angles:
     * Angle 1 (The Lore/Hype): If it's a specific lore/in-game term (Role, Faction, Location), talk about why the NTE community loves this specific theme in the game.
     ...
     * Angle 3 (Direct & Blunt): Start immediately by stating that finding good NTE wallpapers for {$tagName} is hard, but this collection fixes that.
...
7. keywords (comma-separated string):
   CRITICAL ENTITY DATA. Generate a comprehensive list of synonyms, aliases, acronyms, popular nicknames, and international spellings for the tag "{$tagName}" specifically within the NTE community.
   - MUST Include native language writing if applicable (e.g., Chinese Hanzi/Pinyin since NTE is made by a Chinese developer, Japanese Romaji/Kanji for JP dub fans).
   - Include alternative related terms (e.g., if tag is "Vehicle", include "Car", "Tuning", "Racing").
...
8. alt_text_pattern (string):
   ...
   - Example: "{$tagName} [Subject] Neverness to Everness wallpaper in 4K resolution, [Dominant Color] aesthetic"

9. faqs (JSON Array):
   Generate exactly 2 conversational Q&A pairs about "{$tagName}" NTE wallpapers to capture Voice Search traffic.
   - Keep answers under 25 words. Be direct.
   - Example Q: "Are these {$tagName} NTE wallpapers free to download?"
...
public function generateSeriesPrompt(int $id): JsonResponse
{
    ...
    // Berikan fallback spesifik gacha/NTE jika keywords kosong
    $seriesAlias = $series->keywords ?? 'nte, perfect world, urban anime gacha';

    $prompt = <<<EOT
You are an authentic, passionate player and gacha gamer running a personal wallpaper blog. Your writing MUST pass as 100% human-written and bypass AI detectors (score 85%+ human).
...
3. CASUAL AUTHORITY: Write like a real player sharing a massive image dump with friends on Reddit, Discord, or community forums. Be helpful but brief.
...
1. _series_analysis:
   ...
   - If it's an urban fantasy or anime game (like Neverness to Everness or Tower of Fantasy), identify 2-3 iconic characters and 1-2 major locations/factions (e.g., the city of Hethereau, Esper factions, Anomaly zones) from this universe to use dynamically in the text.
...
3. seo_title (50-60 characters):
   ...
   - ABBREVIATIONS: If {$seriesName} has a widely recognized abbreviation (e.g., Neverness to Everness = NTE), use it to save space.
   - TITLE CASE: Capitalize The First Letter Of Every Word.
   - NO SPECIAL CHARACTERS: Strictly DO NOT use (), [], -, or |. Use only spaces to separate words.
...
5. atomic_answer (40-60 words):
   ...
   - Format: "{$seriesName} is a [Format/Genre from step 1] developed by [Developer, e.g., Perfect World / Hotta Studio], recognized for its [Visual Aesthetic/Lore Elements]. This hub page provides a massive gallery of high-resolution desktop and mobile wallpapers featuring main characters and environments from the series."
...
6. description (150-200 words):
   ...
   - Paragraph 1 (The Lore Hook): Explain the appeal of {$seriesName}. Mentally roll a dice and pick ONE angle:
     * Angle 1 (The Meta/Hype): Talk about the community's obsession with the banners, character kits, vehicle tuning, and why everyone wants this specific art as a catalyst for their gacha pulls.
     * Angle 2 (The World-Building): Focus heavily on the atmosphere, the urban anomalies, or the specific universe design. Drop the location/faction names from step 1.
     * Angle 3 (The Pure Aesthetic): Geek out over the specific art style (e.g., urban techwear, cel-shaded anime, customizable cars) and character designs.
